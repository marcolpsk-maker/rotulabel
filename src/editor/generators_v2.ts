...
export function generateQRCodeDataURL(text: string): string {
  ...
  // Padrao de QR Code (Simulado para Mockup Profissional)
  ctx.fillStyle = "#000";
  const padding = 30;
  const dotSize = 8;
  ...
  // Dots aleatorios para parecer um QR real
...
const VD_REFERENCES: Record<string, number> = {
  "Energia": 2000, "Carboidratos": 300, "Acucares": 50, "Proteinas": 50,
  "Gorduras Totais": 55, "Gorduras Saturadas": 22, "Fibra": 25, "Sodio": 2400,
  "Vitamina C": 45, "Vitamina D": 5, "Vitamina B12": 2.4, "Zinco": 7, "Calcio": 1000,
};
...
export function generateNutritionTableSVG(
  ...
  <text x="8" y="44" font-family="Arial" font-size="10" fill="#000">Porcoes por embalagem: ${data.servingsPerPackage ?? "-"}</text>
  <text x="8" y="60" font-family="Arial" font-size="10" fill="#000">Porcao: ${data.servingSize ?? "-"}</text>
  <rect x="0" y="${headerH}" width="${w}" height="${colsH}" fill="#000"/>
  <text x="8" y="${headerH + 20}" font-family="Arial" font-size="9" font-weight="bold" fill="#fff">Informacoes por porcao</text>
  <text x="${c1W + 8}" y="${headerH + 20}" font-family="Arial" font-size="9" font-weight="bold" text-anchor="middle" fill="#fff">Qtd/Porcao</text>
